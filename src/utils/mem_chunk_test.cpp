...
int main()
{
	auto chunk = utils::ts_mem_chunk<>{};

	constexpr const auto chunk_len = 4096u;
	using elem_type = std::array<std::uint8_t, chunk_len>;
	const auto kLen = 1'000'00;//0;
	...
	auto producer = std::thread{[&]
	{
		...
		{
			chunk.put(v.data(), v.size());
		}
		chunk.put(nullptr, 0u);
	...
	{
		//auto got = chunk.get_for(v.data(), v.size(), 1ms);
		auto it = std::begin(consumer_flow);
		while (true)
		{
			if (auto got = chunk.get(it->data(), it->size()); got == 0)
			{
				break;
			}
			++it;
	...
	{
		std::cerr << i << "\n";
		for (auto val : producer_flow[i])
		{
  			std::cerr << (int)val << " ";
		}
		std::cerr << "\n";
		for (auto val : consumer_flow[i])
		{
			std::cerr << (int)val << " ";
		}
